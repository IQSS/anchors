...
insert <- function(data,
  ...
  if (!inherits(obj, "anchors.rank") && !inherits(obj, "anchors.rank.type"))
    stop("Second argument, obj, must be of class 'anchors.rank' or 'anchors.rank.type' \n")

  if (inherits(obj, "anchors.rank") && inherits(obj$rank, "anchors.rank.type"))
    obj <- obj$rank
  ...
  insert.anchors.rank.type( data, obj$span, overwrite, vnames , debug=debug)
}
