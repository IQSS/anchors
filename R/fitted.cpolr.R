...
fitted.cpolr <- function(object, anchors, average = FALSE, unconditional = FALSE,  ...) {
  ...
  if (!missing(anchors) && !inherits(anchors, "anchors.rank") && !inherits(anchors, "anchors.rank.type"))
    stop("Second argument, anchors, must be of class 'anchors.rank' or 'anchors.rank.type' \n")

  if (!missing(anchors) && inherits(anchors, "anchors.rank") && inherits(anchors$rank, "anchors.rank.type"))
